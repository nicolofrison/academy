...
function func(query, argomenti) {
    return new Promise((resolve, reject) => {
      connection.query(query, argomenti, (error, results, fields) => {
...
app.put('/users/:input', [
  ...
  try {
    ...
    const x = await func('SELECT COUNT(*) AS rowsNumber FROM Users WHERE email=?', [req.params.input, req.params.input]);
    console.log(x);
    if (x.length > 0) {
      await func('UPDATE Users SET  name=?, surname=?, birth_date=?, password=?, newsletter=? WHERE email=? ',
        [req.body.name, req.body.surname, req.body.birth_date, req.body.password,
          req.body.newsletter, req.params.input]);
    } else {
